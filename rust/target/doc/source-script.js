...
function createDirEntry(elem, parent, fullPath, currentFile, hasFoundFile) {
  ...
  if (elem.dirs) {
    for (var i = 0; i < elem.dirs.length; ++i) {
      if (createDirEntry(elem.dirs[i], folders, fullPath, currentFile, hasFoundFile) === true) {
...
function createSourceSidebar() {
  ...
  Object.keys(sourcesIndex).forEach(function (key) {
    ...
    hasFoundFile = createDirEntry(sourcesIndex[key], sidebar, '', currentFile, hasFoundFile)
  })

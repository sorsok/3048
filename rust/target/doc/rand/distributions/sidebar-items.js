initSidebarItems({
  mod: [
    ['range', 'DEPRECATED: Use `distributions::uniform` instead.'],
  ...
  struct: [
    ...
    ['Binomial', 'The binomial distribution `Binomial(n, p)`.'],
    ['Cauchy', 'The Cauchy distribution `Cauchy(median, scale)`.'],
    ['ChiSquared', 'The chi-squared distribution `χ²(k)`, where `k` is the degrees of freedom.'],
    [
      'DistIter',
      'An iterator that generates random values of `T` with distribution `D`, using `R` as the source of randomness.',
    ],
    ['Exp', 'The exponential distribution `Exp(lambda)`.'],
    [
      'Exp1',
    ...
    ['FisherF', 'The Fisher F distribution `F(m, n)`.'],
    ['Gamma', 'The Gamma distribution `Gamma(shape, scale)` distribution.'],
    ['LogNormal', 'The log-normal distribution `ln N(mean, std_dev**2)`.'],
    ['Normal', 'The normal distribution `N(mean, std_dev**2)`.'],
    [
      'Open01',
    ...
    ['Poisson', 'The Poisson distribution `Poisson(lambda)`.'],
    [
      'Standard',
    ...
    ['StudentT', 'The Student t distribution, `t(nu)`, where `nu` is the degrees of freedom.'],
    ['Uniform', 'Sample values uniformly between two bounds.'],
    ['Weighted', 'A value with a particular weight for use with `WeightedChoice`.'],
  ...
  trait: [
    ...
    ['IndependentSample', '`Sample`s that do not require keeping track of state.'],
    ['Sample', 'Types that can be used to create a random instance of `Support`.'],
  ],
})

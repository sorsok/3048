...
;(function () {
  ...
  function handleHashes(ev) {
    ...
    if (savedHash !== window.location.hash) {
      ...
      var elem = document.getElementById(savedHash.slice(1))
    ...
    function execQuery(query, searchWords, filterCrates) {
      ...
      function extractGenerics(val) {
        ...
        if (val.indexOf('<') !== -1) {
          ...
          return { name: val.substring(0, val.indexOf('<')), generics: values.split(/\s*,\s*/) }
        }
  ...
  function createSimpleToggle(sectionIsCollapsed) {
    ...
    toggle.innerHTML =
      '[<span class="inner">' + labelForToggleButton(sectionIsCollapsed) + '</span>]'
    return toggle
  }
  ...
  newToggle.innerHTML =
    '[<span class="inner">' + labelForToggleButton(true) + '</span>] Show hidden undocumented items'
  function toggleClicked() {
    if (hasClass(this, 'collapsed')) {
      removeClass(this, 'collapsed')
      onEachLazy(
        this.parentNode.getElementsByClassName('hidden'),
        function (x) {
      ...
      this.innerHTML =
        '[<span class="inner">' + labelForToggleButton(false) + '</span>] Hide undocumented items'
    } else {
      addClass(this, 'collapsed')
      onEachLazy(
        this.parentNode.getElementsByClassName('x'),
        function (x) {
          if (hasClass(x, 'content') === false) {
            addClass(x, 'hidden')
            removeClass(x, 'x')
  ...
  onEachLazy(document.getElementsByClassName('impl-items'), function (e) {
    ...
    for (var i = 0; i < hlength; ++i) {
      if (
        hasClass(hiddenElems[i], 'content') === false &&
  ...
  function showModal(content) {
    ...
    modal.innerHTML =
      ...
      '</div><div class="whiter"></div><span class="docblock">' +
      content +
      '</span></div>'
  ...
  function buildHelperPopup() {
    ...
    var shortcuts = [
      ...
      .map((x) => '<dt><kbd>' + x[0] + '</kbd></dt><dd>' + x[1] + '</dd>')
      .join('')
